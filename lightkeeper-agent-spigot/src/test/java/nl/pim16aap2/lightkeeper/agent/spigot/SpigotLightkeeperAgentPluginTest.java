...
class SpigotLightkeeperAgentPluginTest
{
    ...
    @Test
    void onCommand_shouldReturnFalseWhenSenderIsNotAPlayer()
        throws Exception
    {
        // setup
        ...
        when(command.getName()).thenReturn("lktestgui");

        // execute
        final boolean handled = plugin.onCommand(sender, command, "lktestgui", new String[0]);
    ...
    @Test
    void onCommand_shouldReturnFalseWhenCommandNameDoesNotMatch()
        throws Exception
    {
        // setup
        final SpigotLightkeeperAgentPlugin plugin = allocatePlugin();
        ...
        // execute
        final boolean handled = plugin.onCommand(player, command, "other", new String[0]);
    ...
    @Test
    void onCommand_shouldOpenMainMenuWhenDispatcherExists()
        throws Exception
    {
        // setup
        final SpigotLightkeeperAgentPlugin plugin = allocatePlugin();
        ...
        // execute
        final boolean handled = plugin.onCommand(player, command, "lktestgui", new String[0]);

        // verify
        assertThat(handled).isTrue();
        verify(dispatcher).openMainMenu(player);
    }

    @Test
    void onCommand_shouldReturnFalseWhenDispatcherIsUnavailable()
        throws Exception
    {
        // setup
        final SpigotLightkeeperAgentPlugin plugin = allocatePlugin();
    ...
    @Test
    void onInventoryClick_shouldDelegateWhenDispatcherExists()
        throws Exception
    {
        // setup
        final SpigotLightkeeperAgentPlugin plugin = allocatePlugin();
        ...
        // execute
        plugin.onInventoryClick(event);

        // verify
        verify(dispatcher).onInventoryClick(event);
    }
