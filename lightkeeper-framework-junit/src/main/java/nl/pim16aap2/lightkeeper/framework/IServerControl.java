 ...
 *
 * <p>Obtained from {@link ILightkeeperFramework#server()}.
 */
public interface IServerControl
{
     ...
     * server directory may be modified freely — see {@link #directory()}.
     *
    ...
    /**
     * Restarts the Minecraft server: a graceful {@link #stop()} when it is running, followed by {@link #start()}.
     *
     * <p>Also valid when the server is already down (after {@link #stop()} or {@link #crash()}), in which case it
     * only starts the server. See {@link #start()} for what is — and is not — restored.
     */
    void restart();
     ...
     * gaps: exceptions that are caught and never logged are invisible, as are log events emitted before the agent
     * plugin loads (use {@link #output()} for pre-boot inspection).
     *
     * @return A handle exposing the captured server errors.
     */
    ServerErrorsHandle errors();
}
